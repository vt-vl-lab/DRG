...
def im_detect(model, image_id, Test_RCNN, word_embeddings, object_thres, human_thres, detection, device, opt):
    ...
    for object_out in Test_RCNN[image_id]:
        ...
        if opt['use_thres_dic'] == 1:
            object_thres_ = opt['thres_dic'][object_out[4]]
        else:
            object_thres_ = object_thres

        if (np.max(object_out[5]) > object_thres_):  # This is a valid object
            ...
            for human_in in Test_RCNN[image_id]:

                if (human_in[1] == 'Human') and (np.max(human_in[5]) > human_thres) and not (np.all(human_in[2] == object_out[2])):  # This is a valid object

                    h_box_ = np.array([human_in[2][0], human_in[2][1], human_in[2][2], human_in[2][3]]).reshape(1, 4)
                    h_box = np.concatenate((h_box, h_box_), axis=0)

                    O_vec_ = word_embeddings[object_out[4]]
                    O_vec = np.concatenate((O_vec, O_vec_), axis=0)

                    Pattern_ = generate_spatial(human_in[2], object_out[2]).reshape(1, 2, 64, 64)
                    Pattern = np.concatenate((Pattern, Pattern_), axis=0)

                    human_score = np.concatenate((human_score, np.max(human_in[5]).reshape(1, 1)), axis=0)
                    O_class = np.concatenate((O_class, np.array(object_out[4]).reshape(1, 1)), axis=0)
            ...
            blobs['object_word_embeddings_object_centric'] = torch.FloatTensor(O_vec).reshape(pos_num, 300)
            ...
            for idx in range(len(prediction_HO)):
                temp = []
                temp.append(h_box[idx])  # Human box
                temp.append(object_out[2])  # Object box
                temp.append(O_class[idx])  # Object class
                temp.append(prediction_HO[idx])  # Score
                temp.append(human_score[idx])  # Human score
                temp.append(object_out[5])  # Object score
                This_object.append(temp)

    detection[image_id] = This_object
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/e2e_faster_rcnn_R_50_FPN_1x_HICO_sp_only.yaml",
        metavar="FILE",
