...
def im_detect(model, image_id, Test_RCNN, word_embeddings, object_thres, human_thres, detection, detection_human, detection_object, device, opt):
...
def run_test(
    ...
    for idx, line in enumerate(tqdm(image_list)):
        ...
        if image_id in test_detection:
            im_detect(model, image_id, test_detection, word_embeddings, object_thres, human_thres, detection, detection_human, detection_object, device, opt)
...
def main():
    ...
    for output_folder, dataset_name in zip(output_folders, dataset_names):
        ...
        output_file_object = os.path.join(output_folder, 'detection_object.pkl')
