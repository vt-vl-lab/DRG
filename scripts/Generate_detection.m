function Generate_detection(model)
...
iter=0;
if strcmp(model,'finetune')
    out_file = ['../../output/HICO/matlab/HICO_finetune/mat'];
else
    out_file = ['../../output/HICO/matlab/HICO/mat'];
end

string=[model '_' num2str(iter)];
...
for obj_idx = 1 : 80
    if strcmp(model,'finetune')
        Detection = load(['../../output/HICO/matlab/HICO_finetune/detections_' sprintf('%02d',obj_idx) '.mat']);
    else
        Detection = load(['../../output/HICO/matlab/HICO/detections_' sprintf('%02d',obj_idx) '.mat']);
    end
    ...
    for i = 1 : size(Detection,1)
        ...
        HOI     = Detection{i,4}; % 0~599
        score   = Detection{i,5};

        HOI = HOI - start + 2;

        if isempty(all_boxes{HOI, find(list == imageId)}) %#ok<*FNDSB>
    ...
    save_mat(obj_idx, out_file, all_boxes)
end

% delete(gcp('nocreate'))

eval_run([out_file '/'], iter, string)
